...
4. **Fine-tune** Qwen3-4B using QLoRA on the training split
...
"""## Step 6 — Load Qwen3-4B-Instruct-2507 with 4-bit Quantization (QLoRA Setup)

### QLoRA
- **Q** = Quantization: load the base model in **4-bit** to save GPU memory (~2.5GB instead of ~6GB)
- **LoRA** = Low-Rank Adaptation: instead of updating all 4B parameters, we add small trainable adapter matrices to specific layers
