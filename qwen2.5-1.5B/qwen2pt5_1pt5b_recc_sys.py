...
4. **Fine-tune** Qwen2.5-1.5B using QLoRA on the training split
...
"""## Step 6 — Load Qwen2.5-1.5B with 4-bit Quantization (QLoRA Setup)

### QLoRA
- **Q** = Quantization: load the base model in **4-bit** to save GPU memory (~2.5GB instead of ~6GB)
- **LoRA** = Low-Rank Adaptation: instead of updating all 1.5B parameters, we add small trainable adapter matrices to specific layers
